...
import { createReader, type Entry, type Reader } from "@keystatic/core/reader";
import { compile } from "@mdx-js/mdx";
import serialize from "serialize-javascript";

import keystaticConfig from "../test/fixtures/keystatic.config";
...
>(params: BuildParams<Collections, Singletons>): Promise<void> {
	...
	for (const [key, collection] of Object.entries(reader.config.collections ?? {})) {
		...
		for (const id of await collectionReader.list()) {
			...
			imports.push(`import ${_id} from "./${fileName}";`);
			exports.push(`"${id}": ${_id}`);
		}
